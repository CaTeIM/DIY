...
File manager self-hosted que serve arquivos existentes do HD diretamente na interface web, sem importação ou banco de índice. Acesso externo seguro via Cloudflare Tunnel em `https://files.exemplo.com`.
...
1. Acessar `http://192.168.x.x:5244`
...
> O registro CNAME `files.exemplo.com` é criado automaticamente no Cloudflare DNS.
...
1. Abrir `https://files.exemplo.com`
...
curl -u admin:SuaSenha https://files.exemplo.com/dav/ -X PROPFIND
```
