...
```
   ...
   │   :8443 -> (futuro)               │ criptografado│  │ serve HTTPS por porta  │  │
   └───────────────────────────────────┘              │  └───────────┬────────────┘  │
                                              ┌───────┘              │               │
                                              │ host.docker.internal │     proxy     │
                                              │                      ▼               │
                                              │           Portainer :9443 (no host)  │
                                              └──────────────────────────────────────┘
...
### Como o container alcança cada backend

- **Serviço na rede docker** (ex.: um app na `caddy-net`) → o container está na mesma rede: `Proxy: http://<container>:porta`.
- **Serviço publicado no host** (ex.: Portainer em `0.0.0.0:9443`) → via `host.docker.internal` (o `extra_hosts: host-gateway` da stack resolve o IP do host): `Proxy: https+insecure://host.docker.internal:9443`. O `+insecure` aceita o certificado self-signed do Portainer; o Tailscale põe um cert `*.ts.net` válido na frente.
...
```json
{
  ...
  "Web": {
    "${TS_CERT_DOMAIN}:443": {
      "Handlers": { "/": { "Proxy": "https+insecure://host.docker.internal:9443" } }
    }
  }
}
```

> ℹ️ `${TS_CERT_DOMAIN}` é trocado **pelo container** pelo FQDN do node (`selflabs.<tailnet>.ts.net`) — não hardcode.
...
docker logs tailscale 2>&1 | grep -iE 'serve|https'
```
...
- **Node como servidor:** use `tag:server` (Parte 2) — não expira, não conta como device pessoal. Se criou o node sem tag, aplique em Machines → node → `⋯` → **Edit ACL tags** → `tag:server`, e **Disable key expiry**.
- 🔒 **Fechar a exposição pública do Portainer:** hoje o Portainer publica em `0.0.0.0:9443` (aberto na internet). Com a tailnet no ar, você pode **restringir** — mas há um porém: se trocar para `127.0.0.1:9443`, o `host.docker.internal` deixa de alcançá-lo. A forma limpa é pôr o Portainer na **`caddy-net`** e trocar o proxy para `http://portainer:9000` (rede docker, sem publicar porta). Aí o painel fica **só** na tailnet.
...
O mesmo node serve vários serviços, um por **porta**. Ex.: adicionar um Grafana (que vive na `caddy-net`):
...
1. **serve.json** — adicione a porta (`8443`) e o backend:
   ```json
   {
     ...
     "Web": {
       "${TS_CERT_DOMAIN}:443":  { "Handlers": { "/": { "Proxy": "https+insecure://host.docker.internal:9443" } } },
...
| `502`/página em branco | backend errado no serve.json, ou `extra_hosts` faltando | Confira o `Proxy`; para serviço no host, precisa do `host.docker.internal:host-gateway` |
| Portainer dá erro de conexão | proxy `http://` num Portainer só-HTTPS | Use `https+insecure://host.docker.internal:9443` |
...
- **Hub por porta:** um node serve vários serviços (Parte 7). Para URLs sem porta, use um node dedicado por serviço.
...
- [Portainer (este repo)](./portainer-debian.md)
