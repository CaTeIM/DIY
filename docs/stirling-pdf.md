...
> [!NOTE]
> **Hoje o `pdf.selflabs.org` deste ambiente roda o [BentoPDF](./bentopdf.md), não o Stirling.** Este
> guia continua aqui como **alternativa e referência**: ele segue válido e funcional, e é o caminho
> a seguir se você quiser um app com **backend próprio** (o processamento acontece no **servidor**,
> não no navegador de quem usa). A escolha entre os dois é essa:
>
> - **Stirling-PDF** (este guia): Java + LibreOffice + Tesseract **no servidor**. Pede RAM no host e
>   o upload do arquivo até o container, mas funciona em qualquer navegador e não depende de CDN.
> - **[BentoPDF](./bentopdf.md)**: nginx estático, processamento **no navegador** via WebAssembly.
>   Servidor quase sem carga e o arquivo nunca sai da máquina de quem usa, mas o trabalho pesado
>   passa a depender da RAM do cliente.
>
> Como os dois disputam o mesmo hostname, **rode um de cada vez** e aponte o
> [Caddyfile](../assets/configs/caddy-Caddyfile) para o container correspondente ([Parte 4](#parte-4-autenticação-authelia-e-exposição-externa)).
...
- No **[Caddyfile](../assets/configs/caddy-Caddyfile)** já existe o bloco de `pdf.selflabs.org` com `import authelia`, mas o `reverse_proxy` aponta para **`bentopdf:8080`** (o app que ocupa esse hostname hoje). Para usar o Stirling, troque a linha para `reverse_proxy stirling-pdf:8080` e rode `docker restart caddy` (o `caddy reload` não funciona nesta stack: o `admin off` desliga a API na `:2019`).
- No **[Authelia](../assets/configs/authelia.yml)** já existe a regra `pdf.selflabs.org` no `access_control` (por padrão `one_factor` = só senha; troque para `two_factor` se quiser exigir TOTP também). Essa regra vale para qualquer app nesse hostname, então **não muda** ao alternar entre Stirling e BentoPDF.
