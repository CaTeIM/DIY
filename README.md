...
| **[🔀 Reverse Proxy (Caddy + forward-auth)](./docs/reverse-proxy.md)** | Caddy compartilhado que roteia por hostname e impõe login (Authelia) nos apps protegidos. Base para reusar auth em vários projetos. | `Caddy`, `Docker`, `Cloudflare Tunnel`, `forward-auth` |
